...
# is the one branch that still runs a byte at a time. The "vs library" column
# is vestigial now that the library is the fast one.
#
...
def show(title, keys):
    ...
    for k in keys:
        ...
        if base is None:
            base = ms
        print('    %-8s %6d ms   %5.1f cycles/byte   %5.2fx'
              % (k, ms, cyc, base / ms if ms else 0))
